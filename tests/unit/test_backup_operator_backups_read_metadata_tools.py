# ruff: noqa: S105
"""Curated Backup read + metadata tests (list, get, set_labels, set_annotations).
...
# rancher_backup_set_annotations (PatchConfig substrate — cluster-scoped, no namespace)
# ======================================================================================


class StubBackupSetAnnotationsClient:
    """Patch-capable raw Kubernetes proxy stub for the set_annotations tests.

    Captures the most recent ``patch_json`` request so tests can
    assert on the merge-patch body and path (cluster-scoped — no namespace
    segment), then echoes the backup payload back with updated annotations.
    """
...
@pytest.mark.asyncio
async def test_rancher_backup_set_annotations_round_trip() -> None:
    """PATCH body must be {metadata: {annotations: <dict>}} at the cluster-scoped detail path."""

    reset_rate_limit_state()
    client = StubBackupSetAnnotationsClient()

    result = await rancher_backup_set_annotations(
        backup_name="weekly-backup",
...
@pytest.mark.asyncio
async def test_rancher_backup_set_annotations_emits_audit() -> None:
    """Audit record must carry operation='backup_set_annotations'."""

    reset_rate_limit_state()

    with capture_logs() as logs:
        await rancher_backup_set_annotations(
            backup_name="weekly-backup",
            annotations={"app": "rancher-backup"},
            cluster_id="local",
            instance="work",
    ...
    assert len(audit_records) == 1
    record = audit_records[0]
    assert record["tool_name"] == "rancher_backup_set_annotations"
    assert record["operation"] == "backup_set_annotations"
    assert record["plane"] == "steve"
    assert record["outcome"] == "success"

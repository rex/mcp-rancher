"""Curated PodMonitor delete tool tests (destructive).

Covers PodMonitor delete at ``monitoring.coreos.com/v1``.
"""
...
from rancher_mcp.tools.prometheus_monitoring import rancher_pod_monitor_delete


class StubPodMonitorDeleteClient:
    """Delete-capable raw Kubernetes proxy stub for the pod_monitor delete tests.
    ...
    async def delete_json(
        ...
        if path == detail_path:
            ...
            return {
                ...
                "apiVersion": "v1",
                "status": "Success",
                "details": {"name": "demo-pod-monitor", "kind": "podmonitors"},
            }
...
@pytest.mark.asyncio
async def test_rancher_pod_monitor_delete_refuses_wrong_confirmation() -> None:
    """Wrong confirmation phrase must raise RancherCapabilityError before any HTTP call."""

    reset_rate_limit_state()
    client = StubPodMonitorDeleteClient()

    with pytest.raises(RancherCapabilityError) as excinfo:
        await rancher_pod_monitor_delete(
    ...
    assert "delete pod_monitor demo-pod-monitor in namespace monitoring" in str(excinfo.value)
    assert client.last_delete_path is None


@pytest.mark.asyncio
async def test_rancher_pod_monitor_delete_routes_to_delete_json_on_correct_phrase() -> None:
    """Correct confirmation phrase routes to delete_json on the detail path."""

    reset_rate_limit_state()
    client = StubPodMonitorDeleteClient()

    result = await rancher_pod_monitor_delete(
        namespace="monitoring",
        pod_monitor_name="demo-pod-monitor",
        confirmation="delete pod_monitor demo-pod-monitor in namespace monitoring",
        cluster_id="local",
        instance="work",
        settings=build_settings(),
        client=client,
    ...
    assert result.resource_kind == "pod_monitor"
    assert result.resource_name == "demo-pod-monitor"
    assert result.namespace == "monitoring"
    assert result.cluster_id == "local"
    assert result.confirmation_phrase_used == (
        "delete pod_monitor demo-pod-monitor in namespace monitoring"
    )
    assert result.response_payload["kind"] == "Status"
    assert result.response_payload["status"] == "Success"
    assert result.suggested_next_steps == ["rancher_pod_monitors_list"]


@pytest.mark.asyncio
async def test_rancher_pod_monitor_delete_emits_audit_with_outcome() -> None:
    ...
    assert success_audits[0]["operation"] == "pod_monitor_delete"
    ...
    assert reject_audits[0]["operation"] == "pod_monitor_delete"

...
# catalog/curated_tools/groups.yml. Do not edit by hand.
...
async def rancher_groups_list(
    ...
    """List groups with typed summaries."""
...
async def _fetch_group_get(
    ...
    """Fetch and normalize one group."""

    payload = await client.get_json(f"/v3/groups/{group_id}")

    detail = RancherGroupDetail.model_validate(payload)
    return detail.model_copy(
        update={
            "link_keys": sorted(mapping_value(payload, "links") or {}),
            "payload": dict(payload),
            "suggested_next_steps": ["rancher_groups_list", "rancher_users_list"],
        }
    )
...
async def rancher_group_get(
    ...
    """Fetch one group by name."""

    resolved_settings = settings or get_settings()
    instance_name, instance_config = resolve_instance(resolved_settings, instance)
    if client is not None:
        return await _fetch_group_get(instance_name, group_id, client)
    async with RancherManagementClient(instance_name, instance_config) as managed_client:
        return await _fetch_group_get(
            instance_name,
            group_id,
            managed_client,
        )
...
async def rancher_groups_list_tool(
    ...
    """List groups as lightweight typed summaries — identity, state, and a per-item health rollup rather than full specs — so an agent can enumerate what exists before opening any one in detail with the matching get tool."""
...
async def rancher_group_get_tool(
    ...
    """Fetch one group and return its full typed detail: the conditions, diagnostics, and derived fields the list summary leaves out. Call this once a list has identified which one to inspect."""

    return await rancher_group_get(
        group_id=group_id,
        instance=instance,
    )

...
async def rancher_pod_disruption_budgets_list(
    ...
    """List pod_disruption_budgets with typed summaries — in one namespace, or cluster-wide when namespace is omitted."""
...
async def _patch_pod_disruption_budget_set_labels(
    ...
    """Set_labels one pod_disruption_budget via JSON merge-patch; returns a mutation receipt."""
    ...
    before = await fetch_patch_before(
        lambda: client.get_json(pdb_resource_path(cluster_id, namespace, budget_name)),
    ...
    await client.patch_json(
        pdb_resource_path(cluster_id, namespace, budget_name), payload=request_payload
    )
...
async def _patch_pod_disruption_budget_set_annotations(
    ...
    """Set_annotations one pod_disruption_budget via JSON merge-patch; returns a mutation receipt."""
